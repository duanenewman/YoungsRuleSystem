...
namespace Amendment.Server.IdentityStores
{
    public class UserStore : IUserStore<User>, IUserRoleStore<User>, IUserPasswordStore<User>, IUserSecurityStampStore<User>
    {
        ...
        public Task RemoveFromRoleAsync(User user, string roleName, CancellationToken cancellationToken)
        {
            ...
            var role = user.Roles.FirstOrDefault(ur => ur.Name == roleName);
            if (role != null)
        ...
        {
            if (user == null) throw new ArgumentNullException(nameof(user));
            IList<string> output = user.Roles.Select(ur => ur.Name).ToList();
        ...
        public Task<bool> IsInRoleAsync(User user, string roleName, CancellationToken cancellationToken)
        {
            if (user == null) throw new ArgumentNullException(nameof(user));
            if (roleName == null) throw new ArgumentNullException(nameof(roleName));
            return Task.FromResult(user.Roles.Any(ur => ur.Name == roleName));
        }
